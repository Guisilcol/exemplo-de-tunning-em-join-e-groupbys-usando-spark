...
"""
    Create test data for the weather station temperature
"""
import os
import sys
import random
...
WEATHER_STATIONS_FILEPATH: str = os.getenv('WEATHER_STATIONS_FILEPATH')
...
def build_weather_station_name_list():
    ...
    """
    station_names = []
    with open(WEATHER_STATIONS_FILEPATH, 'r') as file:
...
def build_test_data(weather_station_names, num_rows_to_create):
    ...
    try:
        with open(MEASUREMENTS_FILEPATH, 'w') as file:
            progress = 0
    ...
    file_size = os.path.getsize(MEASUREMENTS_FILEPATH)
    human_file_size = convert_bytes(file_size)
 
    print(f"Test data successfully written to {MEASUREMENTS_FILEPATH}")
